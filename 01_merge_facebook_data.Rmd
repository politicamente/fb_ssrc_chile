...
df_politicamente_tweets <- read_csv(
  ...
  count(twitter_id) %>% 
  rename(n_tweets = n) %>% 
  left_join(df_politicamente_cands %>% select(candidato_id, twitter_id))
```
...
Finally, we add the number of tweets to our dataset and save it:

```{r}
df_facebook_candidate <- df_facebook_candidate %>% 
  left_join(df_politicamente_tweets %>% select(candidate_id = candidato_id,
                                               n_tweets))
