...
Now we are going to group the data at the candidate level for future analysis. This dataset only includes the candidates from the districts 10, 11 and 13, which are the focus of this paper.
...
First we summarise number of psots by candidate and then we summarise the number of posts of each type (from codif_macro_1) by candidate
```{r error=FALSE, message=FALSE, warning=FALSE}
...
The final step is merging with candidate level variables from `01_merge_facebook_data_post.rds` and save the new candidate level dataset
```{r error=FALSE, warning=FALSE, message=FALSE}
df_facebook <- df_facebook %>% 
